...
class Processor:
    ...
    def start(self, input_video, output_video, upscaling_factor, upscaling_model, interpolation_factor, interpolation_model, cugan_denoise, output_format, gpuid):
        try:
            ...
            self.__thread_writer = Writer(self.__output_format, self.__output_video, self.__output_queue, self.__fps * self.__interpolation_factor, self.__width * self.__upscaling_factor, self.__height * self.__upscaling_factor, self.__frame_count * self.__interpolation_factor, self.__socket).start_writing()
            ...
            if self.__upscaling_factor > 1:
                if not self.__upscaler or self.__upscaler.gpuid != self.__gpuid or self.__upscaler.upscaling_factor != self.__upscaling_factor or (self.__upscaler.denoise != self.__cugan_denoise) or (self.__upscaler.model_name != self.__upscaling_model):
            ...
            if self.__interpolation_factor > 1:
                if not self.__interpolater or self.__interpolater.UHD != self.__UHD or self.__interpolater.interpolation_model != self.__interpolation_model or (self.__interpolater.gpuid != self.__gpuid):
